...
test("@visual Today morning brief mobile progressive disclosure", async ({ page }) => {
  await page.setViewportSize({ width: 430, height: 932 });
  await installRichCopilotRoute(page);
  await page.goto("/");
  await page.getByRole("button", { name: "Open Jordan Rivera morning brief" }).first().click();
  ...
  await expect(page).toHaveScreenshot("morning-brief-mobile-collapsed.png");
  ...
  await brief.evaluate((element) => element.scrollIntoView({ block: "start" }));
  await expect(page).toHaveScreenshot("morning-brief-mobile-expanded.png");
});

test("@visual expanded Movement graph mobile branch", async ({ page }) => {
  await page.setViewportSize({ width: 430, height: 932 });
  ...
  await expect(page).toHaveScreenshot("movement-graph-expanded-mobile-430.png", { fullPage: true });
});

test("@visual expanded member-context graph mobile branch", async ({ page }) => {
  await page.setViewportSize({ width: 430, height: 932 });
  ...
  await expect(page).toHaveScreenshot("member-context-graph-expanded-mobile-430.png", { fullPage: true });
});
