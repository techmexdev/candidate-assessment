...
Completion is one Neo4j write transaction. Before completion, the fenced worker stores three immutable checkpoint artifacts on the run: the two revision-seal references, the canonical graph-traversed safety envelope, and the parsed model proposal. A reclaimed lease clears these generation-scoped artifacts and rebuilds them from the pinned sources. The repository recomputes their canonical SHA-256 digests at the transaction boundary and requires the receipt plus the candidate-set and model-proposal provenance entities to match; receipt-only digests are never trusted. It also verifies run ownership, authorization reference, current fence, state, request digest, pinned revisions, resolved-constraint digest, complete decision evidence, workout identity, and provenance shape before creating the immutable workout, decisions, PROV links, receipt, and one terminal event. Cancellation wins when its transaction changes the run before the completion compare-and-set. Duplicate completion with the same workout version is a read-only replay.

## Idempotency, replay, and retention

The database uniqueness key is `(coachId, memberId, action, idempotencyKeyDigest)`. Submission and retry first atomically `MERGE` a `WorkoutRunReservation` for that scope and request digest; only its lease owner may provision the durable grant reference or protected prompt and promote the reservation to a queued `WorkoutRun`. Concurrent exact duplicates wait for that promotion and replay the same run, while changed-payload reuse conflicts before either provisioning boundary is called. An abandoned owner releases the reservation or loses its bounded lease; recovery keeps the reservation's original run ID, creation timestamp, and provisioning key so idempotent grant/prompt adapters return the same artifacts instead of allocating replacements. A reservation is not a readable or claimable run, and final promotion (input revision plus queued event) is one database transaction, so partial provisioning never exposes a usable run. A new idempotency key may create a new run for the same logical request.
